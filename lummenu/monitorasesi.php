...
else if ($op == "validasi")
{
	...
	$execpemetaan=mysql_query($sqlpemetaan);
	$listpemetaan = mysql_fetch_array($execpemetaan);
	$idpasse=$listpemetaan['idasesor'];
	$nmass=$listpemetaan['namaasesor'];
	//echo $execpemetaan;
	$lttd="SELECT * FROM lsp_usertbl WHERE id_user='".$idpasse."'";
	$resultxttd=mysql_query($lttd);
	$hasilxttd=mysql_fetch_array($resultxttd);
	$ttd=$hasilxttd['linkttd'];
	$namaxttd=$hasilxttd['nama'];
   ...
   $adax=mysql_fetch_array($ada0);
    ...
    {
    echo "Nama Peserta : ".$namaadsesi."</br>";
	echo "Nama Asesor : " .$nmass."</br>";
...
<?php
    ...
    echo "<td><input type=checkbox name=bukti[] value='sk'$kbu0 disabled='disabled'>SR
		...
		<input type=checkbox name=bukti[] value='l' $kbu7 disabled='disabled'>L</td>";
    		echo"<td><a href=../siswa/gambarimages/".$path." target=_blank> $path Tampilkan Dokumen </a></td>";
...
//echo $cekapl2;
									...
									echo"<td><input type=checkbox name=validasia".$i." value='v' $kdbs0>V<input type=checkbox name=validasib".$i." value='a' $kdbs1>A<input type=checkbox name=validasic".$i." value='t' $kdbs2>T <input type=checkbox name=validasid".$i." value='m' $kdbs3>M</td><td><a href=\"".$_SERVER['PHP_SELF']."?op=deleteapl2buka&idapl2=".$hcekapl2['id']."\"><span class='glyphicon glyphicon-remove'>Hapus</a></td>";
									     }else{
					...
					$sqlpemetaan="select * from pemetaan where idpeserta='$idadsesi'";
					$execpemetaan=mysql_query($sqlpemetaan);
					$listpemetaan = mysql_fetch_array($execpemetaan);
					$idpasse=$listpemetaan['idasesor'];
					//echo $execpemetaan;
...
else if ($op == "deleteapl2buka")
{
	//echo "abc".$_GET['idapl2'];
	$idapl2=$_GET['idapl2'];
	$cekdelap2buka="select id from apl2 where id='$idapl2'";
	//echo $cekdelap2buka;
	$cekdelap2bukaa=mysql_query($cekdelap2buka);
	$cekdelap2bukab=mysql_num_rows($cekdelap2bukaa);
	if ($cekdelap2bukab>0)
    {
	echo "<form id=formContoh method=\"post\" action=\"".$_SERVER['PHP_SELF'].
        "?op=deletepostapl2buka\">";

    echo "<input type=hidden name=idapl2buka value=".$idapl2.">";
          ?>
	 
    	Yakin  Akan di hapus ? 
        
        <?php
       echo "<input type=submit id=gobutton value=Delete></br>";   
    echo "</form>";
...
else if ($op == "deletepostapl2buka")

{
	$idapl2del=$_POST['idapl2buka'];
	$delapl2buka="delete from apl2 where id='$idapl2del'";
	//echo $delapl2buka;
	$delapl2bukaa=mysql_query($delapl2buka);
	if($delapl2bukaa)
	{ echo "Berhasil ..";}
	else 
    { echo "Gagal ...";}
...
$idasesi= $_GET['idasesi'];
	...
	echo "<table><tr>";
	echo "<td colspan=3 bgcolor='#A9D0F5'><strong>Kode Unit</strong></td>";
	echo "<td bgcolor='#A9D0F5'><strong>Hasil Tes</strong></td><td bgcolor='#A9D0F5'><strong>Keterangan</strong></td><td bgcolor='#A9D0F5'><strong>Aksi</strong></td></tr>";
	$i=0;
	while ($rowgrade=mysql_fetch_array($exegrade)){
		...
		if($rowgrade['grade']<100)
		 ...
		 $ket="Kompeten";}
		echo "<tr>";
		echo "<td><input type=checkbox name=idgrade".$i." value='$gid'></td>";
...
else if ($op=="deletegnol"){
//echo "tasyatyast";
	$n=$_POST['n'];
	for ($i=0; $i<=$n-1; $i++)
	{
    if (isset($_POST['idgrade'.$i]))
     {
	...
	echo "<font color=green size=4><span class='glyphicon glyphicon-floppy-saved'><strong> $asesor </strong></font>";}
	else{echo "<font color=red><span class='glyphicon glyphicon-floppy-remove'> Belum di Jadwalkan</font>";}

	echo "</br><font color=blue>BIODATA</font></br>";
	if(mysql_num_rows($execsqlapl1)>0){
	echo "<font color=green> <span class='glyphicon glyphicon-floppy-saved'> Sudah terdaftar dengan nama : ".$rowapl1['namasiswa']." </font></br><font color=green> Email : $rowapl1a</font>";
	}
	...
	echo "<font color=green><span class='glyphicon glyphicon-floppy-saved'> Sudah Mengisi Tanggal Registrasi : ".$rowpermohonan['tanggal']."</font>";

	}

	}else { echo "<font color=red><span class='glyphicon glyphicon-floppy-remove'> Belum terdaftar</font>";}

	$sqlupload="select buktiapl1,email from apl1 where email='$email' group by email";
	//echo $sqlupl
	$execsqlupload=mysql_query($sqlupload);
	$execsqluploadb=mysql_fetch_array($execsqlupload);
	echo "</br><font color=blue>APL1 Dengan Bukti</font></br>";
	if(strlen($execsqluploadb['buktiapl1'])>0){
	//while ($rowupload=mysql_fetch_array($execsqlupload)){

	echo "<font color=green><span class='glyphicon glyphicon-floppy-saved'> Sudah Upload bukti</font>";
  ...
  //}
	...
	echo "<font color=green><span class='glyphicon glyphicon-floppy-saved'> Sudah Mengisi Tanggal Registrasi :  ".$rowapl2['waktu']."</font>";
	}
	}else { echo "<font color=red><span class='glyphicon glyphicon-floppy-remove'> Belum terdaftar</font>";}

	echo "</br><font color=blue>PORTOFOLIO</font></br>";
	$porto="Select email,waktu from upload where email='$email' and waktu='$tglpem'";
	//echo $porto;
	$portoa=mysql_query($porto);
	$portob=mysql_num_rows($portoa);
	//echo $portob;
	if($portob>0)
	{
	echo "<font color=green><span class='glyphicon glyphicon-floppy-saved'>Sudah Mengisi </font>";}
	else{echo "<font color=red><span class='glyphicon glyphicon-floppy-remove'>Belum Mengisi </font>";}

	echo "</br><font color=blue>OBSERVASI</font></br>";
	$sqlobs="select * from rekappraktek where idadsesi='$iduser'";
	$exeobs=mysql_query($sqlobs);
	//echo $sqlobs;
	if(mysql_num_rows($exeobs)>0){
	$rowobs=mysql_fetch_array($exeobs);
	//$asesor=$rowjadwal['namaasesor'];

	echo "<font color=green><span class='glyphicon glyphicon-floppy-saved'> Pernah di observasi </font>";}
	else{echo "<font color=red><span class='glyphicon glyphicon-floppy-remove'> Belum Pernah di observasi</font>";}

	$mak5="select idasesi from mak5 where idasesi='$email' group by idasesi";
	$mak5a=mysql_query($mak5);

	echo "</br>";
	echo "<font color=blue>UMPAN BALIK MAK4</font>";
	if(mysql_num_rows($mak5a)>0){
   	while ($mak5b = mysql_fetch_array($mak5a)){
        echo "</br>";
        echo " <a href=viewmak5.php?emaila='".$email."' target=_blanlk> <font color=green><span class='glyphicon glyphicon-floppy-saved'> Sudah Mengisi</font></a>";
	}	
	} else { echo "</br>";echo "<font color=red><span class='glyphicon glyphicon-floppy-remove'> Belum Mengisi</font>";}
	echo "</br>";

	$ckrekomedasii ="Select namarekom,idskema,idasesi,rekom,catatan from rekomendasi where idskema='$dskema' and idasesi='".$iduser."' and namarekom='apl1lsp'";
	//echo $ckrekomedasii;
	$ckrekomedasiia=mysql_query($ckrekomedasii);
	$ckrekomedasiib=mysql_fetch_array($ckrekomedasiia);
	$catatani=$ckrekomedasiib['catatan'];
	//echo $ckrekomedasib['rekom']; 
	echo "</br>";
	echo "<font color=blue><strong>Rekomendasi APL1</strong></font> ";
	if (mysql_num_rows($ckrekomedasiia)) {
	echo "</br>";
	if($ckrekomedasiib['rekom']=='L')
		{
		...
		echo "</br> <strong>Catatan : ".$catatani."</strong></br>";
}

	$ckrekomedasiiapl2 ="Select namarekom,idskema,idasesi,rekom,catatan from rekomendasi where idskema='$dskema' and  idasesi='".$iduser."' and namarekom='apl2'";
	$ckrekomedasiiaapl2=mysql_query($ckrekomedasiiapl2);
	$ckrekomedasiibapl2=mysql_fetch_array($ckrekomedasiiaapl2);
	$catatanapl2i=$ckrekomedasiibapl2['catatan'];
	echo "</br>";
	echo "<font color=blue><strong>Rekomendasi APL2</strong></font> ";
	if (mysql_num_rows($ckrekomedasiiaapl2)) {
	echo "</br>";
	if($ckrekomedasiibapl2['rekom']=='L')
		{
		echo "<font color=green ><span class='glyphicon glyphicon-ok'><strong> Ket : Diterima </strong></font>"; 
		} else 
		{
		...
		echo "</br> <strong>Catatan : ".$catatanapl2i."</strong>";
	}
...
{
	echo "<strong>";
	//echo "<form method=POST action='daftarpes.php'>
	echo "<form id=\"formContoh\" method=\"post\" action=\"".$_SERVER['PHP_SELF'].
        "?op=daftarpes\">";
    
	echo "
        Filter Tanggal :<select name =tgl id=tgl >";
		$listmodul=mysql_query("SELECT tanggal FROM settanggal where status='A'");
...
else if ($op == "daftarpes")
{
	...
	echo "<table width=90%>";
	echo "<tr><td bgcolor='#006699' colspan=7><strong>Tanggal : $tanggal</strong></td></tr>";
	echo "<tr><td bgcolor='#006699'><strong>ID User (Asesi) </strong></td><td bgcolor='#006699'><strong>Nama Asesi</strong></td><td bgcolor='#006699'><strong>Permohonan </strong></td><td bgcolor='#006699'><strong>APL 1 </strong></td><td bgcolor='#006699'><strong>Portofolio</strong> </td><td bgcolor='#006699'><strong>APL 2</strong> </td><td bgcolor='#006699'><strong>Umpan Balik</strong></td></tr>"; 
	$i=0;
	while ($data=mysql_fetch_array($hasil)){
	$iduser=$data["id_user"];
	$nama=$data["nama"];
	$email=trim($data["email"]);
	$nmdrpemetaan="select namaasesor,idpeserta from pemetaan where idpeserta='$iduser'";
	//echo $nmdrpemetaan;
	$nmdrpemetaana=mysql_query($nmdrpemetaan);
	$nmdrpemetaanb=mysql_fetch_array($nmdrpemetaana);
	$nmasesor=$nmdrpemetaanb['namaasesor'];

	$qpermohonan="select email, tanggal from permohonan where email='$email' and tanggal='$tanggal' group by email,tanggal";
	$qpermohonana=mysql_query($qpermohonan);
	if(mysql_num_rows($qpermohonana)>0){
	$p1="<font color=green><span class='glyphicon glyphicon-ok'>Sudah Terdaftar</font>";
	}else{
	$p1='<font color=red>Belum Terdaftar</font>';}
	$qapl1="select email,namasiswa from apl1 where email='$email' group by email";
	$qapl1a=mysql_query($qapl1);

	if(mysql_num_rows($qapl1a)>0){
        $p3="<font color=green><span class='glyphicon glyphicon-ok'>Sudah Terdaftar</font>";
    } 
    else { 
     $p3=" <font color=red>Belum Terdaftar</font>";
	}


	$qupload="select email,waktu from upload where email='$email' and waktu='$tanggal' and idskema='$skem' group by email,waktu";
	$quploada=mysql_query($qupload);
	
	if(mysql_num_rows($quploada)>0){
   	while ($quploadb = mysql_fetch_array($quploada)){
    
        $cekvlapl1="select idasesi,idskema,waktu,statusvalid from upload where email='".$quploadb['email']."' and waktu='".$quploadb['waktu']."' and statusvalid='Y' group by idasesi,idskema,waktu,statusvalid";
       ...
       if($b>0){
          ...
          else{$ketvl="<font color=red><span class='glyphicon glyphicon-remove'>Belum divalidasi</font>";}
          //echo ",".$ketvl;
         }	
		} else { //echo "</br>";
	$ketvl= "<font color=red>Belum Terdaftar</font>";}


	//$cekvlapl2="select idadsesi,idskema,waktu,svalidasi from apl2 where idskema='$skem' and waktu='$tanggal' and email='".$email."' and svalidasi='T' group by idadsesi,idskema,waktu,svalidasi";
	$cekvlapl2="select idadsesi,idskema,waktu,svalidasi from apl2 where waktu='$tanggal' and email='".$email."' and svalidasi='T' group by idadsesi,idskema,waktu,svalidasi";
      //echo $cekvlapl2;
     ...
     if($b>0){
	      ...
	      //$cekvlaplabc="select idadsesi,idskema,waktu,svalidasi from apl2 where idskema='$skem' and waktu='$tanggal' and email='".$email."' and svalidasi='Y' group by idadsesi,idskema,waktu";
          	$cekvlaplabc="select idadsesi,idskema,waktu,svalidasi from apl2 where waktu='$tanggal' and email='".$email."' and svalidasi='Y' group by idadsesi,idskema,waktu";
	  //    echo $cekvlaplabc;
   ...
   $umpan="select idasesi,tglreg from mak5 where idasesi='$email' and tglreg='$tanggal'";
   //echo $umpan;
   $umpana=mysql_query($umpan);
   $umpanb=mysql_num_rows($umpana);
   if($umpanb>0)
   { 
        $ketumpan="<font color=green><span class='glyphicon glyphicon-ok'>Sudah Mengisi</font>";}
		else{$ketumpan="<font color=green><span class='glyphicon glyphicon-search'>Asesi belum mengisi</font>";}

	echo "<tr><td>$iduser </td><td>$nama</br>Asesor : $nmasesor</td><td>$p1</td><td>$p3</td><td>$ketvl</td><td>$ketvl2</td><td>$ketumpan</td></tr>";
...
<input type='hidden' name='n' value='".$i."' />
<div class='buttons'>
<tr><td colspan=2><input id='gobutton' type='submit' value='Simpan' /></td></tr>*/
echo "</table>";
}
echo "</form>";
}
...
<?php echo "<a href=exportbuatlog.php class='btn btn-primary btn-sm' role='button'> <span class='glyphicon glyphicon-plus'></span><strong>Export Laporan Asesmen</strong></a>";?>

<?php //echo "<a href=\"".$_SERVER['PHP_SELF'].
        //"?op=uploadgbr\" class='btn btn-primary btn-sm' role='button'> <span class='glyphicon glyphicon-plus'></span><strong>Upload Bukti</strong></a>";?>
<?php echo " <a href=\"".$_SERVER['PHP_SELF'].
        "?op=pumum"."\"  class='btn btn-primary btn-sm' role='button'> <span class='glyphicon glyphicon-plus'><strong>Pemantauan Pertanggal </strong> </a>";?><br/><br/>


<table align="left" class='demo-table' width="50%">
<tr><td><input type="button" value="REFRESH" onclick="window.location='monitorasesi.php'">
<form method="post" action="monitorasesi.php">
...
<table  class='demo-table' width="97%">
<th bgcolor='#006699' align="center"> NO </th>
<th bgcolor='#006699' align="center"> ID Asesi</th>
<th bgcolor='#006699' align="center"> NAMA </th>
<!--<th bgcolor='#006699' align="center"> Biodata </th>-->
<th bgcolor='#006699' colspan="7" align="center"> Aksi </th>
...
$sql="select * from lsp_usertbl where level='peserta' order by id_user DESC LIMIT $offset, $dataPerPage";
}
...
echo "<td><a href=\"".$_SERVER['PHP_SELF'].
        "?op=validasi&idasesi=".$row[0]."&nama=".$row[1]."\"><span class='glyphicon glyphicon-book'>Permohonan</a>";
echo "<td><a href=\"".$_SERVER['PHP_SELF'].
...
echo "<td><a href=\"".$_SERVER['PHP_SELF'].
        ...
        echo "<td align=left><a href=../siswa/biodatasiswapdf.php?email=".$row[2]." target=_blank><span class='glyphicon glyphicon-user'>Biodata</a></td>";
echo "<td><a href=\"".$_SERVER['PHP_SELF'].
