...
echo "<form id=\"formContoh\" method=\"post\" action=\"".$_SERVER['PHP_SELF'].
        "?op=simpansett\">";
?>
	<table class=demo-table width=100%><thead><tr>
	<td><strong>Tanggal    </strong></td><td> : 
    <input type="text" name="tanggal" id="tanggal" placeholder="Tanggal"></td></tr><tr>
	<td><strong>Keterangan </strong></td><td> :
    <input type="text" name="ket" id="ket" placeholder="Keterangan" size="80"></td></tr><tr>
...
else if($op=="simpansett")
{
	...
	$cekdata = "SELECT * FROM settanggal WHERE tanggal = '$tanggal'";
	   $ada=mysql_query($cekdata);
		if(mysql_num_rows($ada)>0){
			 echo "duplikate..";}
			else{
		$ssql="insert into settanggal value('','$tanggal','$ket','$yt')";
...
{
	$tgl=$_GET['idtgl'];
	$hapus="delete from settanggal where idtanggal='$tgl'";
	$ehapus=mysql_query($hapus);
	//echo $hapus;
		if($ehapus){ echo "sukses...";}
			  else{echo "gagal ...";}
	}
else if($op=="editsett")
	{
	...
	<table><tr>
	<td><strong>Tanggal    </strong></td><td> : 
	<input type="text" name="tanggal" id="tanggal" placeholder="Tanggal" value="<?php echo $tanggal;?>"></td></tr><tr>
	<td><strong>Keterangan </strong></td><td> :
	<input type="text" name="ket" id="ket" placeholder="Keterangan" size="80" value="<?php echo $ket;?>" ></td></tr><tr>
	<td><strong>Status </strong></td><td> :
	<input type="radio" name="yt" value="a" <?php if ($st=='A') echo checked  ?>> Aktif <input type="radio" name="yt" value="t" <?php if ($st=='T') echo checked  ?>> Tidak Aktif </td></tr><tr>
	<td colspan="2"><input type="submit" name="simpan" id="gobutton" value="Simpan"><input type="hidden" name="id" value="<?php echo $idtgl;?>"></td></tr></table>
	<?php
	}
	...
	$up="update settanggal set tanggal='$tanggal', ket='$ket', status='$yt' where idtanggal='$idt'";
	//echo $up;
	$eup=mysql_query($up);
		if($eup){ echo "sukses...";}
			  else{echo "gagal ...";}
...
$no = 1;
$querysetmain ="SELECT * FROM settanggal";
$hasilsetmain = mysql_query($querysetmain);
while ($datasetmain = mysql_fetch_array($hasilsetmain))
{
	  if($datasetmain['status']=='A') {
		...
		echo "<td bgcolor='#99CCFF' align=left>".$datasetmain['tanggal']."</td>";
		echo "<td bgcolor='#99CCFF' align=left>".$datasetmain['ket']."</td>";
		...
		echo "<td><a href=\"".$_SERVER['PHP_SELF'].
			"?op=editsett&st=".$datasetmain['status']."&tanggal=".$datasetmain['tanggal']."&ket=".$datasetmain['ket']."&idtgl=".$datasetmain['idtanggal']."\"><span class='glyphicon glyphicon-pencil'>Edit</a> | <a href=\"".$_SERVER['PHP_SELF'].
			"?op=hapussett&idtgl=".$datasetmain['idtanggal']."\"><span class='glyphicon glyphicon-remove'>Hapus</a></td>";
		echo "</tr>";
		$no++;
}
   echo "</div>";
