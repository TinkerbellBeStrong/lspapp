...
?>
	...
	<div id="sidebar-collapse" class="col-sm-3 col-lg-2 sidebar">
		...
		<ul class="nav menu">
			...
			<li class="active"><a href="inputtempattuk.php"><svg class="glyph stroked home"><use xlink:href="#stroked-home"/></svg> Kelola Tempat TUK</a></li>
...
if ($op == "edittuk")
{
   ...
   $hasiltukedit = mysql_query($querytukedit);
   $datatukedit  = mysql_fetch_array($hasiltukedit);
   
   $namatuk=$datatukedit['namatuk'];   
	echo "<form id=\"formContoh\" method=\"post\" action=\"".$_SERVER['PHP_SELF'].
        "?op=updatetuk\">";
	echo "<table border=\"0\">"; 
?>
	<div class="form-group"><strong>Nama TUK :</strong> 
	<input type="text" name="namatuk" id="namatuk" placeholder="Nama TUK" class="form-control" value="<?php echo $namatuk; ?>" autofocus required/>
	</div>
   <hr>
   <input type="hidden" name="idtuk" value="<?php echo $idtuk; ?>">
...
?>
  ...
  <hr>
  <div class="form-group"><strong>Nama TUK :</strong>
  <input type="text" name="namatuk" id="namatuk" placeholder="Nama TUK" class="form-control" autofocus required/>
</div>
...
else if ($op == "appendtuk")
     {
      $namatuk=$_POST['namatuk'];
      
	  $query = "INSERT INTO tuk VALUE('','$namatuk')";
      echo $query;
...
else if ($op == "deletetuk"){

   $idtuk= $_GET['idtuk'];
   $query = "SELECT * FROM tuk WHERE idtuk = '$idtuk'";
   $hasil = mysql_query($query);
   $data  = mysql_fetch_array($hasil);
   $idtuk=$data['idtuk'];
   $namatuk=$data['namatuk'];
 //echo "<form name='myform' method='post' action='".$_SERVER['PHP_SELF']."??op=edit'>";
   echo "<form id=formContoh method=\"post\" action=\"".$_SERVER['PHP_SELF'].
        "?op=deleteposttuk\">";
         echo "<input type=hidden name=idtuk value=".$idtuk.">";
          ?>
	 <div class="formcolumn">
    	Yakin TUK : <strong><?php echo $namatuk ?></strong>  Akan di hapus ? 
        <div class="buttons">
        <?php
...
<?php echo "<a href=\"".$_SERVER['PHP_SELF'].
        "?op=tambahtuk&kd_modul=".$data['kd_modul']."\" class='btn btn-primary btn-sm' role='button'> <span class='glyphicon glyphicon-plus'></span>Tambah TUK</a>";?>
<table class=demo-table width=100%><thead>
<tr>
    <th bgcolor='#006699'>No</th>
    	
    <th bgcolor='#006699'>ID TUK</th>
...
$hasiltukmain = mysql_query($querytukmain);
while ($datatukmain = mysql_fetch_array($hasiltukmain))
{

   echo "<tr>";
   echo "<td bgcolor='#99CCFF' align=left>".$no."</td>";
   //echo "<td bgcolor='#99CCFF'>".$data['id_user']."</td>";
   echo "<td bgcolor='#99CCFF' align=left>".$datatukmain['idtuk']."</td>";
   echo "<td bgcolor='#99CCFF' align=left>".$datatukmain['namatuk']."</td>";
  
  	
   echo "<td align=left><a href=\"".$_SERVER['PHP_SELF'].
        "?op=edittuk&idtuk=".$datatukmain['idtuk']."\"><span class='glyphicon glyphicon-pencil'>Edit</a> | <a href=\"".$_SERVER['PHP_SELF'].
        "?op=deletetuk&idtuk=".$datatukmain['idtuk']."\"><span class='glyphicon glyphicon-remove'>Hapus</a></td>";
   echo "</tr>";
   $no++;
}
   echo "</div>";
