...
?>
	...
	<div id="sidebar-collapse" class="col-sm-3 col-lg-2 sidebar">
		...
		<ul class="nav menu">
			...
			<li class="active"><a href="inputprosesasesmen.php"><svg class="glyph stroked pen tip"><use xlink:href="#stroked-pen-tip"/></svg>Input Proses Asesmen</a></li>
	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">			
		<div class="row">
			<h4><strong>Pengeloaan Komponen Proses Asesmen</strong></h4>
...
if ($op == "editsyaratu")
{
  ...
  <label for="syaratu">Komponen Umpan Balik </label>
	...
	else if ($op == "tambahkpa")
	{
		...
		Komponen proses asesmen </th></tr><tr><td>
 		
		<tr><td>
		<label for="kpa">Komponen Proses Asesmen</label>   
  		<input id="kpa" name="kpa" placeholder="Komponen proses asesmen" type="text" size="70">
    	</td></tr><tr><td>
		<div class="buttons">
		<input id="gobutton" type="submit" value="Simpan " />
		</div>
...
else if ($op == "appendkpa")
     {
      //$idskema=$_POST['skema'];
      $syaratkpa=$_POST['kpa'];
      //$kodesya=$_POST['kodesya'];
      	if (!empty($syaratkpa)){
      			$queryuappendsyaratkpa = "INSERT INTO qmak7 VALUE('','$syaratkpa')";
				 // echo $queryuappendsyarat;
				  $berhasilsyaratkpa = mysql_query($queryuappendsyaratkpa);
				  if ($berhasilsyaratkpa) { ?>
		<div class="container">
...
{
   $idsyaratkpa= $_GET['idkpa'];
   $queryueditkpa = "SELECT * FROM qmak7 where idqmak7 = '$idsyaratkpa'";
   //echo $queryueditkpa;
   $hasilueditkpa = mysql_query($queryueditkpa);
   $dataueditkpa  = mysql_fetch_array($hasilueditkpa);
   $nmsyaratkpa = $dataueditkpa['pertanyaan'];
   $idskema=$dataueditkpa['idqmak7'];
   //$idsyaratu=$dataueditsyu['idsyaratu'];	
   echo "<form method=\"post\" action=\"".$_SERVER['PHP_SELF'].
        "?op=updatesyaratkpa\">";
		?>
		<table class=demo-table width=80%><tr><td>
		<label for="syaratkpa">Komponen Proses Asesmen </label>
		<input id="syaratkpa" name="syaratkpa" placeholder="Komponen proses asesmen" type="text" size="90" value="<?php echo $nmsyaratkpa;?> ">
		</td></tr></table>

		<?php
		echo "</table>";
   		echo "<input type=\"hidden\" name=\"idsyaratkpa\"
         value=\"".$dataueditkpa['idqmak7']."\">";
   		?>
		<div class="buttons">
		<input id="gobutton" type="submit" value="Simpan " />
		</div>
...
else if ($op == "updatesyaratkpa")
     {
		// proses untuk updating data setelah diedit
        $idsyaratekpa = $_POST['idsyaratkpa'];
        $syaratekpa= $_POST['syaratkpa'];
		// $skema=$_POST['skema'];
		//  $kdsyarat=$_POST['kdsyarat'];
		$queryuupdatesyaratkpa = "UPDATE qmak7 SET pertanyaan='$syaratekpa' WHERE idqmak7 = '$idsyaratekpa'"; 
        //echo $queryuupdatesyaratkpa;
        $hasiluupdatesyaratkpa = mysql_query($queryuupdatesyaratkpa);
        if ($hasiluupdatesyaratkpa) 
        { ?>
		<div class="container">
        <div class="alert alert-success">
          <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
...
else if ($op == "deletekpa"){
   		$idsyaratkpa= $_GET['idkpa'];
   		$queryudelsyaratkpa = "SELECT * FROM qmak7 where idqmak7 = '$idsyaratkpa'";
   		$hasiludelsyaratkpa = mysql_query($queryudelsyaratkpa);
   		$dataudelsyaratkpa  = mysql_fetch_array($hasiludelsyaratkpa);
	    $nmsyaratkpa=$dataudelsyaratkpa['pertanyaan'];
		//echo $queryudelsyaratu;
	    //$namaunit=$data['namaunit'];
		//echo "<form name='myform' method='post' action='".$_SERVER['PHP_SELF']."??op=edit'>";
	    echo "<form method=\"post\" action=\"".$_SERVER['PHP_SELF'].
		    "?op=deletepostsyaratkpa\">";
		echo "<input type=hidden name=idsyaratkpa value=".$idsyaratkpa.">";
          ?>
	 	<table class=demo-table width=80%><tr><td>
          Komponen :  <?php echo $nmsyaratkpa ; ?> <font color="red">Yakin Akan di hapus ??</font>
		</td></tr><tr><td>
...
else if ($op == "deletepostsyaratkpa"){
		$sqldelsyaratkpa="DELETE from qmak7 WHERE idqmak7=".$_POST['idsyaratkpa'];
		//echo $sqldelsyaratu;
		mysql_query($sqldelsyaratkpa);
		if($sqldelsyaratkpa)
		{
			?>
		<div class="container">
...
<body>
	<!--<?php echo "<a href=\"".$_SERVER['PHP_SELF'].
        "?op=tambahkomponenu\" class='btn btn-primary btn-sm' role='button'> <span class='glyphicon glyphicon-plus'></span>Tambah Komponen Proses Asesment</a>";?>-->

	<table class=demo-table></thead><tr>
    <th colspan=5 align=left ><?php echo "<a href=\"".$_SERVER['PHP_SELF'].
        "?op=tambahkpa\" class='btn btn-primary btn-sm' role='button'> <span class='glyphicon glyphicon-plus'></span>Tambah Komponen Proses Asesment</a>";?></th></tr><tr>
...
// bagian ini digunakan untuk menampilkan semua data
	...
	$no = 1;
	$querykpa ="SELECT * from qmak7";
	$hasilkpa = mysql_query($querykpa);
	while ($datakpa = mysql_fetch_array($hasilkpa))
		{
      	   echo "<tr>";
		   echo "<td bgcolor='#99CCFF'>".$no."</td>";
		   echo "<td bgcolor='#99CCFF'>".$datakpa['pertanyaan']."</td>";
		   
		   echo "<td >
		<a href=\"".$_SERVER['PHP_SELF'].
        "?op=editkpa&idkpa=".$datakpa['idqmak7']."\"><span class='glyphicon glyphicon-pencil'>Edit </a>|<a href=\"".$_SERVER['PHP_SELF'].
        "?op=deletekpa&idkpa=".$datakpa['idqmak7']."\"><span class='glyphicon glyphicon-remove'>Hapus</a></td>";
		   echo "</tr>";
			   $no++;
		}
	   echo "</div>";
